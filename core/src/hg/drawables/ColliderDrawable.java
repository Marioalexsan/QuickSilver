...
public class ColliderDrawable extends Drawable {
    ...
    public void draw(SpriteBatch batch) {
        ...
        switch (phase) {
            case 0 -> {
                color.r += phaseSpeed;
                color.g = HgMath.ClampValue(color.g - 2 * phaseSpeed, 0.5f, 1f);
                color.b = HgMath.ClampValue(color.b - 2 * phaseSpeed, 0.5f, 1f);
                if (color.r > 1f) {
                    color.r = 1f;
                    phase = 1;
                    phaseRetain += 1f;
            ...
            case 1 -> {
                color.g += phaseSpeed;
                color.r = HgMath.ClampValue(color.r - 2 * phaseSpeed, 0.5f, 1f);
                color.b = HgMath.ClampValue(color.b - 2 * phaseSpeed, 0.5f, 1f);
                if (color.g > 1f) {
                    color.g = 1f;
                    phase = 2;
